...
#define MIN_REACTION_MS 10000
#define MAX_REACTION_MS 30000
#define CHANCE_TO_SUBMIT_WORD 50 
#define CHANCE_BOT_TENTAR_PALAVRA_ERRADA 10 
...
BOOL carregarDicionarioBot() {
    ...
    while (g_NumeroDePalavrasNoDicionario < MAX_PALAVRAS_DICIONARIO && _fgetts(linhaBuffer, _countof(linhaBuffer), fp) != NULL){
...
DWORD WINAPI threadAdivinharPalavraComDicionario(LPVOID lpParam) {
    ...
    if (g_NumeroDePalavrasNoDicionario == 0)
        LOG_DEBUG(_T("Dicionário vazio ou não carregado. Encerrando thread de adivinhar palavras."));
    ...
    while (g_run_bot) {
        ...
        if (bg->hMutex_mp == NULL || WaitForSingleObject(bg->hMutex_mp, INFINITE) == WAIT_OBJECT_0) {
            ...
            if (_tcslen(palavra_final_para_submeter) > 0) {
                if ((rand() % 100) < CHANCE_BOT_DIGITAR_ERRADO) {
                    ...
                    _tprintf(_T("[BOT:%s] Oops! Preparou '%s', mas 'digitou errado': '%s'\n"), bg->bot_name, (encontrou_palavra_valida_no_dicionario && pPalavraOriginalDoDicionario ? pPalavraOriginalDoDicionario : _T("palavra_aleatoria")), palavra_final_para_submeter);
                }
...
void cleanup_bot_wrapper(void) {
	if (g_bot_globals_for_atexit == NULL) return;
...
int _tmain(int argc, TCHAR* argv[]) {
    ...
    if (ReadFile(bg_local_main.hPipe, respostaLogin, (sizeof(respostaLogin) / sizeof(TCHAR) - 1) * sizeof(TCHAR), &bytesLidosLogin, NULL) && bytesLidosLogin > 0) { respostaLogin[bytesLidosLogin / sizeof(TCHAR)] = _T('\0');
